...
class HierarchicalMesh(object):
    ...
    def cut_with_plane(self, plane):
        ...
        :param plane: vtk plane with origin and normal
        :return:
        """
        if self.mesh is not None:
            cut = vtk.vtkCutter()
            cut.SetInputData(self.mesh.GetMapper().GetInput())
            cut.SetCutFunction(plane)
            cut.Update()
            self.generated_by_plane_cut = cut.GetOutput()

            index = 0
            # for mesh in self.generated_by_plane_cut:
            util.writeStl(self.generated_by_plane_cut, "cut_thing.stl")
                # index = index + 1

        for child in self.children:
            child.cut_with_plane(plane)
